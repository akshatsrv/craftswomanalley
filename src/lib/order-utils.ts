...
export function buildCustomerEmail(customer: Record<string, string>, items: OrderItem[], total: number, orderId: string, estimatedDelivery: string): string {
...
export function buildAdminEmail(customer: Record<string, string>, items: OrderItem[], total: number, orderId: string): string {
  const itemsHtml = items.map((item) => `
